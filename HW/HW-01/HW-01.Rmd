...
logit_roc <- rocit(score = as.vector(df_train$p_hat_EARTH), class = df_train$INS)
plot(logit_roc)
...
ROC Curve
```{r}
df_train$p_hat_GAM <- predict(final.gam, type = "response")
logit_roc <- rocit(score = as.vector(df_train$p_hat_GAM), class = df_train$INS)
plot(logit_roc)
plot(logit_roc)$optimal
paste("AUC =",logit_roc$AUC)
```
